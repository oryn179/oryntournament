...
const Vote: React.FC = () => {
  ...
  const handleFakeLogin = () => {
    const fakeUser: User = {
      username: 'GitHubUser' + Math.floor(Math.random() * 1000),
  ...
  const handleGitHubLogin = () => {
    // Actual GitHub OAuth Redirect
    const redirectUri = window.location.origin + window.location.pathname + '#/vote';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 py-8">
        ...
        {!user ? (
          <GlassCard className="max-w-md mx-auto text-center py-8">
            <p className="text-gray-400 mb-6 font-mono text-sm">
              Please sign in with GitHub to verify your account and cast your vote.
            </p>
            <GlowButton onClick={handleGitHubLogin} className="flex items-center justify-center space-x-2 mx-auto">
          ...
          <div className="flex flex-col items-center space-y-4 mb-8">
            <div className="flex items-center space-x-3 glassmorphism px-4 py-2 border-neon/50">
              <div className="w-3 h-3 bg-neon rounded-full animate-pulse"></div>
              <span className="text-neon font-mono text-sm tracking-widest">LOGGED IN AS: {user.username.toUpperCase()}</span>
            </div>
            {votedId && (
              <p className="text-neon/70 font-mono text-xs animate-pulse">VOTING COMPLETE - ACCESS LOCKED</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {editors.map((editor) => (
          <GlassCard key={editor.id} className={`flex flex-col items-center p-8 transition-transform ${votedId === editor.id ? 'border-neon ring-1 ring-neon ring-opacity-50 scale-105 shadow-[0_0_30px_rgba(57,255,20,0.2)]' : ''}`}>
            <div className="relative mb-6">
              <img src={editor.avatar} alt={editor.name} className="w-24 h-24 rounded-full border-2 border-neon/30 grayscale hover:grayscale-0 transition-all duration-500" />
              {votedId === editor.id && (
                <div className="absolute -bottom-2 -right-2 bg-neon text-black rounded-full p-1 shadow-lg">
            ...
            <div className="text-neon font-mono text-lg mb-6">{editor.votes} <span className="text-[10px] text-gray-500 uppercase tracking-[0.2em]">VOTES</span></div>
            ...
            <GlowButton 
              ...
              className="w-full"
            >
              {votedId === editor.id ? 'VOTED' : votedId ? 'LOCKED' : 'VOTE'}
            </GlowButton>
          </GlassCard>
        ))}
      </div>
      
      <div className="text-center text-gray-500 font-mono text-[10px] tracking-[0.3em] uppercase pt-12">
        One account = One vote | Security verified by GitHub OAuth
      </div>
    </div>
  );
};
